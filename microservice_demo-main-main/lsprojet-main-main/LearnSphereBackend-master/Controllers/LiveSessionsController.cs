...
public class LiveSessionsController : ControllerBase
{
    ...
    public LiveSessionsController(AppDbContext db, IFileUploadService fileUpload)
    {
        _db = db;
    ...
    public async Task<IActionResult> JoinSession(int id)
    {
        ...
        // Only track attendance if we are strictly within the Start and End time bounds
        var now = DateTime.UtcNow;
        if (now < session.StartTime || now > session.EndTime)
        {
            return Ok(new { joined = false, message = "Session is not currently live. Attendance is only recorded if joined during the scheduled time." });
        }
    ...
    public async Task<IActionResult> Create([FromForm] LiveSessionCreateDto dto)
    {
        ...
        {
            var result = await _fileUpload.UploadFileAsync(dto.VideoFile, "video", dto.CourseId);
        ...
        {
            var result = await _fileUpload.UploadFileAsync(dto.ThumbnailFile, "image", dto.CourseId);
            if (!result.success) return BadRequest(new { error = result.error });
