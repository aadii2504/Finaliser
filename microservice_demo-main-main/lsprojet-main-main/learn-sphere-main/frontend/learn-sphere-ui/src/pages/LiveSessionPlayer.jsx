    ...
    if (!session) {
        return (
            <div className="flex flex-col items-center justify-center min-h-screen bg-black text-white">
                ...
                <button onClick={() => navigate("/dashboard")} className="mt-4 px-6 py-2 bg-indigo-600 rounded-lg">Go Back</button>
            </div>
        );
    }
    ...
    return (
        <div className="min-h-screen bg-black text-white font-sans">
            <nav className="h-16 bg-[#1e293b] flex items-center px-6 border-b border-white/5">
                <button onClick={() => navigate("/dashboard")} className="p-2 mr-4 hover:bg-white/5 rounded-full transition">
                    <FaArrowLeft />
                </button>
            ...
            <main className="max-w-6xl mx-auto p-6 md:p-12">
                <div className="aspect-video w-full bg-slate-900 rounded-2xl overflow-hidden shadow-2xl relative border border-white/10">
                    {status === "active" ? (
                        session.videoUrl?.startsWith("http") && !session.videoUrl?.includes("/uploads/") ? (
                            <iframe
                        ...
                        <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-8 bg-gradient-to-b from-slate-900 via-black to-slate-900">
                            <div className="w-24 h-24 mb-6 rounded-3xl bg-indigo-500/10 flex items-center justify-center text-5xl shadow-2xl shadow-indigo-500/10 ring-1 ring-indigo-500/20">
                                {status === "upcoming" ? <FaRegClock className="text-indigo-400" /> : <FaPlay className="text-slate-600" />}
                            </div>
                            <h2 className="text-3xl font-black mb-2">
                                {status === "upcoming" ? "Live Session Starts Soon" : "This Session Has Ended"}
                            </h2>
                            {status === "upcoming" ? (
                                ...
                                <p className="text-white/40 max-w-md mx-auto mt-2 italic">
                                    Thanks for joining. The recording will be available shortly in the course library.
                                </p>
                            )}
                        </div>
                    )}
                ...
                <div className="mt-8 grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="bg-white/5 rounded-2xl p-6 border border-white/10 h-fit">
                        <h3 className="text-sm font-black uppercase tracking-widest text-indigo-400 mb-4">Session Details</h3>
                        <div className="space-y-4">
                            <div>
                                <p className="text-[10px] font-black uppercase text-white/30 mb-1">Start Time</p>
                                <p className="font-bold">{formatDateTime(session.startTime)}</p>
                            </div>
                            <div>
                                <p className="text-[10px] font-black uppercase text-white/30 mb-1">Duration</p>
                                <p className="font-bold">
                                    {Math.round((normalizeDate(session.endTime) - normalizeDate(session.startTime)) / 60000)} minutes
                                </p>
                            </div>
                        </div>
                    </div>
