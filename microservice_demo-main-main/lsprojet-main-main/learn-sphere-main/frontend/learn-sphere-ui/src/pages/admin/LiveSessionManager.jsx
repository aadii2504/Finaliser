...
const LiveSessionManager = () => {
    ...
    const openModal = (session = null) => {
        ...
        if (session) {
            ...
            if (session.videoUrl) setVideoPreview({ name: "Current Video", size: "remote" });
            if (session.thumbnailUrl) setThumbPreview({ name: "Current Thumbnail", size: "remote" });
        } else {
    ...
    const FileDropZone = ({ label, type, preview, onFileSelect }) => {
        ...
        return (
            <div className="col-span-2 md:col-span-1">
                <label className="block text-xs font-black uppercase text-white/40 mb-2">{label}</label>
                <div
                    onDragOver={(e) => { e.preventDefault(); setDragActive(true); }}
                    onDragLeave={() => setDragActive(false)}
                    onDrop={(e) => { e.preventDefault(); setDragActive(false); handleFile(e.dataTransfer.files[0]); }}
                    onClick={() => inputRef.current.click()}
                    className={`border-2 border-dashed rounded-xl p-4 text-center cursor-pointer transition-all ${dragActive ? "border-indigo-500 bg-indigo-500/10" : "border-white/10 hover:border-white/20 hover:bg-white/5"
                        }`}
                >
                    <input
                        type="file"
                    ...
                    {preview ? (
                        <div className="flex flex-col items-center">
                            ...
                            <p className="text-[10px] font-bold truncate max-w-full">{preview.name}</p>
                        </div>
                    ) : (
                        <div className="py-2">
                            <p className="text-[10px] text-white/60">Drag & drop or <span className="text-indigo-400">browse</span></p>
                            <p className="text-[8px] text-white/20 mt-1">{type === "video" ? "MP4, MKV..." : "JPG, PNG..."}</p>
                        </div>
                    )}
                </div>
            </div>
    ...
    return (
        <div className="p-6 max-w-7xl mx-auto text-white">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {sessions.map(s => (
                    <div key={s.id} className="bg-white/5 border border-white/10 rounded-2xl overflow-hidden group hover:border-indigo-500/50 transition-all">
                        <div className="h-40 bg-zinc-900 bg-cover bg-center relative" style={{ backgroundImage: `url(${s.thumbnailUrl})` }}>
                        ...
                        <div className="p-4">
                            <div className="flex justify-between items-center text-xs text-white/60 mb-4">
                                ...
                                <span className="text-indigo-400">
                                    {Math.round((normalizeDate(s.endTime) - normalizeDate(s.startTime)) / 60000)} mins
                                </span>
                            </div>
                ...
                <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-zinc-900 border border-white/10 rounded-3xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden shadow-2xl">
                        <div className="p-8 border-b border-white/10 flex justify-between items-center shrink-0">
                            <h2 className="text-2xl font-black">{editingSession ? "Edit Session" : "Schedule Session"}</h2>
                        ...
                        <form onSubmit={handleSubmit} className="p-8 grid grid-cols-2 gap-6 overflow-y-auto">
                            ...
                            <div className="col-span-2">
                                <label className="block text-xs font-black uppercase text-white/40 mb-2">Title</label>
                                <input
                                    required
                            ...
                            <div className="col-span-2">
                                <label className="block text-xs font-black uppercase text-white/40 mb-2">Description</label>
                                <textarea
                            ...
                            <FileDropZone
                                ...
                                onFileSelect={(file) => {
                                    ...
                                    setVideoPreview({ name: file.name, size: (file.size / 1024 / 1024).toFixed(2) + "MB" });
                                }}
                            />
                            ...
                            <FileDropZone
                                ...
                                onFileSelect={(file) => {
                                    ...
                                    setThumbPreview({ name: file.name, size: (file.size / 1024 / 1024).toFixed(2) + "MB" });
                                }}
                            />
                            ...
                            <div className="col-span-2 md:col-span-1">
                                <label className="block text-xs font-black uppercase text-white/40 mb-2">Start Time</label>
                                <input
                                    required
                                    type="datetime-local"
                                    className="w-full bg-black border border-white/10 rounded-xl p-3 focus:border-indigo-500 outline-none transition"
                            ...
                            <div className="col-span-2 md:col-span-1">
                                <label className="block text-xs font-black uppercase text-white/40 mb-2">End Time</label>
                                <input
                                    required
                                    type="datetime-local"
                                    className="w-full bg-black border border-white/10 rounded-xl p-3 focus:border-indigo-500 outline-none transition"
                            ...
                            <div className="col-span-2">
                                <label className="block text-xs font-black uppercase text-white/40 mb-2">Link to Course</label>
                                <select
                                    className="w-full bg-black border border-white/10 rounded-xl p-3 focus:border-indigo-500 outline-none transition"
                            ...
                            <div className="col-span-2 pt-4">
                                <button type="submit" className="w-full py-4 bg-indigo-600 rounded-xl font-bold hover:bg-indigo-500 transition shadow-xl shadow-indigo-600/20">
