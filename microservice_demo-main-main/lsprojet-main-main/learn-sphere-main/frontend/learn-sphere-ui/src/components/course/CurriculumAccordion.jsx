...
export const CurriculumAccordion = ({
    ...
    const getIcon = (type) => {
        switch (type?.toLowerCase()) {
            case "video": return <FaPlayCircle className="w-4 h-4 text-indigo-500" />;
            case "document": return <FaFileAlt className="w-4 h-4 text-emerald-500" />;
            case "image": return <FaImage className="w-4 h-4 text-pink-500" />;
            case "audio": return <FaMusic className="w-4 h-4 text-amber-500" />;
            default: return <FaFileAlt className="w-4 h-4 opacity-40" />;
        }
    };

    if (!chapters || chapters.length === 0) {
        return (
            <div className="p-8 text-center border-2 border-dashed rounded-xl" style={{ borderColor: "var(--border)" }}>
            ...
            {sortedChapters.map((chapter) => (
                ...
                >
                    ...
                    >
                        ...
                        <FaChevronDown className={`w-3 h-3 transition-transform duration-300 ${openChapters[chapter.id] ? "rotate-180" : ""}`} />
                    </button>

                    {openChapters[chapter.id] && (
                        <div className="bg-[#0f172a]/30">
                            {chapter.lessons?.sort((a, b) => (a.order || 0) - (b.order || 0)).map((lesson) => {
                                ...
                                return (
                                    <div
                                        key={lesson.id}
                                        className={`p-4 flex flex-col gap-2 transition-colors border-l-4 ${activeLessonId === lesson.id
                                            ? "bg-white/5 border-indigo-600"
                                            : "hover:bg-white/5 border-transparent"
                                            }`}
                                    >
                                        <div onClick={() => onLessonClick?.(lesson)} className="cursor-pointer">
                                            <div className="flex items-start gap-3">
                                                ...
                                                <div className="mt-0.5 text-indigo-500">
                                                    {
                                                        ...
                                                        (() => {
                                                            const completed = Array.isArray(completedMap[lesson.id]) ? completedMap[lesson.id] : [];
                                                ...
                                                <div className="flex-1 min-w-0">
                                                    <h4 className={`text-sm font-medium leading-tight ${activeLessonId === lesson.id ? "text-white" : "text-white/80"
                                                        }`}>
                                                        {lesson.title}
                                                    </h4>
                                                    ...
                                                    <div className="flex items-center gap-2 mt-2">
                                                        ...
                                                        <span className="text-xs text-white/40">
                                                            {lesson.duration ? `${lesson.duration}min` : "3min"}
                                                        </span>
                                                    </div>
                                                </div>
                                            </div>
                                                ...
                                                {lesson.contents.map((c, idx) => (
                                                    ...
                                                    >
                                                        <div className="flex items-center gap-3 flex-1">
                                                            ...
                                                            <button
                                                                onClick={(e) => { e.stopPropagation(); if (typeof onContentClick === 'function') onContentClick(lesson, idx); else onLessonClick?.(lesson); }}
                                                                className="text-sm text-slate-200 text-left flex-1"
                                                            >
                                                                {c.title || c.contentType}
                                                            </button>
                                                        </div>
                                                        <span className="text-xs text-slate-400 ml-3">{c.contentType}</span>
                                                    </li>
                                                ))}
                                            </ul>
                                        )}
                                ...
                                <div className="p-4 border-t border-slate-700/50 bg-slate-800/30">
                                    ...
                                    <ul className="space-y-2">
                                        {(quizzesByChapter[chapter.id] || []).map((quiz) => {
                                            const passed = Array.isArray(passedQuizIds) && passedQuizIds.includes(quiz.id);
                                            return (
                                                <li key={quiz.id}>
                                                    <button
                                                        type="button"
                                                        onClick={(e) => { e.stopPropagation(); onQuizClick?.(quiz); }}
                                                        className={`w-full flex items-center justify-between p-3 rounded-lg border text-left transition-colors ${activeQuizId === quiz.id
