...
import { FaCheck, FaTimes, FaChevronLeft, FaChevronRight, FaRegClock, FaTrophy, FaRedo, FaLock, FaCheckCircle, FaRegCircle } from "react-icons/fa";
...
export default function AssessmentPlayer({ courseId, onClose }) {
    ...
    const beginSession = (sess, eligData) => {
        ...
        const totalSecs = (eligData?.timeLimitMinutes ?? sess.timeLimitMinutes) * 60;
    ...
    const handleSubmit = async (attemptId, isAutoSubmit = false) => {
        ...
        if (!aid) return;
        if (!isAutoSubmit && !window.confirm("Ready to submit your final assessment?")) return;
    ...
    const handleSelect = (questionId, idx, type) => {
        ...
        if (type === "MCQ") {
            ...
            setAnswers(prev => {
                ...
                return { ...prev, [questionId]: cur.includes(idx) ? cur.filter(x => x !== idx) : [...cur, idx] };
            });
        }
    };
        ...
        return (
            <div className="flex flex-col items-center py-12 text-center max-w-lg mx-auto">
                <div className="w-24 h-24 rounded-3xl bg-white/5 flex items-center justify-center text-4xl mb-6 shadow-inner ring-1 ring-white/10">
                    <FaLock className={isMaxAttempts ? "text-rose-500" : "text-white/20"} />
                </div>
                <h2 className="text-3xl font-black mb-3 text-white">Assessment Locked</h2>
                <p className="text-white/40 mb-10 leading-relaxed px-6">
                    {eligibility?.reason || "Please complete all course requirements to unlock the final assessment."}
                </p>
                ...
                {!isMaxAttempts && (
                    <div className="w-full space-y-6 mb-12 px-2">
                        {eligibility && (
                            <>
                                <ProgBar label="Section Lessons" done={eligibility.lessonsCompleted} total={eligibility.lessonsTotal} />
                                <ProgBar label="Chapter Quizzes" done={eligibility.quizzesPassed} total={eligibility.quizzesTotal} />
                            </>
                        )}
                    </div>
                )}
                ...
                <div className="w-full space-y-4">
                    ...
                    <button onClick={onClose} className="w-full py-4 rounded-xl border border-white/10 hover:bg-white/5 font-bold transition-all text-white/60 hover:text-white">
                        Back to Course
                    </button>
                </div>
            </div>
        ...
        return (
            <div className="flex flex-col py-8 max-w-2xl mx-auto">
                <div className="text-center mb-12">
                    ...
                    <h2 className="text-4xl font-black mb-3 text-white tracking-tight">Final Assessment</h2>
                    <p className="text-emerald-400 font-bold uppercase tracking-widest text-xs">You are qualified for this certification</p>
                </div>

                <div className="grid grid-cols-2 gap-4 mb-10">
                    <div className="bg-white/5 border border-white/10 p-6 rounded-2xl">
                        <p className="text-white/30 text-[10px] uppercase font-black mb-2">Time Limit</p>
                        <p className="text-xl font-bold flex items-center gap-2 text-white">
                            <FaRegClock className="text-indigo-400" /> {eligibility?.timeLimitMinutes} min
                        </p>
                    </div>
                    <div className="bg-white/5 border border-white/10 p-6 rounded-2xl">
                        <p className="text-white/30 text-[10px] uppercase font-black mb-2">Attempts used</p>
                        <p className="text-xl font-bold flex items-center gap-2 text-white">
                            <FaRedo className="text-indigo-400 text-sm" /> {eligibility?.attemptsUsed} of {eligibility?.maxAttempts}
                        </p>
                    </div>
                </div>
                ...
                <div className="space-y-4">
                    ...
                    <button onClick={onClose} className="w-full py-4 text-white/40 font-bold hover:text-white transition-colors">
                        Cancel
                    </button>
                </div>
            </div>
    ...
    if (phase === "started" && session) {
        ...
        const isAnswered = isMulti ? (Array.isArray(selected) && selected.length > 0) : selected !== undefined;

        const timerColor = timeLeft < 120 ? "text-rose-500" : timeLeft < 300 ? "text-amber-500" : "text-emerald-500";
            ...
            <div className="flex flex-col h-full min-h-[600px] animate-in fade-in duration-500">
                ...
                <div className="flex items-center justify-between mb-10 pb-6 border-b border-white/5">
                    <div className="flex items-center gap-4">
                        ...
                        <h3 className="font-black text-sm uppercase tracking-widest text-white/60">Final Assessment</h3>
                    </div>
                    <div className={`px-4 py-2 bg-white/5 rounded-full border border-white/10 flex items-center gap-3 ${timerColor} transition-colors`}>
                ...
                <div className="mb-10">
                    <div className="flex items-center justify-between mb-4">
                        <span className="text-indigo-400 font-bold text-sm tracking-widest uppercase">Question {currentIdx + 1}:</span>
                        {isMulti && <span className="text-[10px] px-2 py-0.5 rounded bg-indigo-500/20 text-indigo-300 uppercase font-black border border-indigo-500/20">Multiple Choice</span>}
                    </div>
                    <h2 className="text-2xl font-bold leading-tight text-white/90">{q.text}</h2>
                </div>
                ...
                <div className="space-y-3 mb-12 flex-1">
                    {opts.map((opt, i) => {
                        ...
                        return (
                            <button key={i} onClick={() => handleSelect(q.id, i, q.type)}
                                className={`w-full flex items-center gap-4 p-5 rounded-xl border text-left transition-all duration-200 group ${isSelected
                ...
                <div className="mt-auto pt-8 border-t border-white/5 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        >
                            {submitting ? "Submit Assessment" : "Submitting..."}
                        </button>
                    </div>
                </div>
            </div>
        ...
        return (
            <div className="flex flex-col items-center justify-center py-16 px-6 text-center animate-in zoom-in duration-500">
                ...
                <div className="flex flex-col items-center gap-1 mb-12">
                    <p className={`text-8xl font-black ${result.passed ? "text-emerald-400" : "text-rose-400"}`}>
                ...
                <div className="grid grid-cols-2 gap-4 w-full max-w-md mb-12">
                    <div className="bg-white/5 rounded-2xl p-4 border border-white/10">
                        <p className="text-[10px] font-black uppercase text-white/30 mb-1">Correct</p>
                        <p className="text-lg font-bold text-white">{result.correct} / {result.total}</p>
                    </div>
                    <div className="bg-white/5 rounded-2xl p-4 border border-white/10">
                        <p className="text-[10px] font-black uppercase text-white/30 mb-1">Attempts Remaining</p>
                        <p className="text-lg font-bold text-white">{result.maxAttempts - result.attemptsUsed}</p>
                    </div>
                </div>
                ...
                <div className="flex gap-4 w-full max-w-sm">
                    ...
                    >
                        {result.passed ? "Claim Your Prize" : "Continue Learning"}
                    </button>
                </div>
            </div>
        );
    ...
    return (
        <div className="w-full">
            <div className="flex justify-between items-center mb-2">
                <div className="flex items-center gap-2">
                    {isDone ? <FaCheckCircle className="text-emerald-400 text-xs" /> : <FaRegCircle className="text-white/20 text-xs" />}
                    <span className="text-xs font-bold text-white/80 uppercase tracking-tight">{label}</span>
                </div>
                <span className="text-[10px] font-black text-white/30 tracking-widest">{done} / {total}</span>
            </div>
            <div className="h-2.5 bg-white/5 rounded-full overflow-hidden border border-white/5">
                <div className={`h-full transition-all duration-1000 ease-out ${isDone ? 'bg-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.3)]' : 'bg-indigo-500 shadow-[0_0_10px_rgba(99,102,241,0.3)]'}`}
                    style={{ width: `${pct}%` }} />
            </div>
        </div>
    );
    ...
    return (
        <div className="w-full bg-white/5 rounded-2xl border border-white/10 overflow-hidden shadow-2xl">
            <div className="p-4 bg-white/5 border-b border-white/10">
                <h3 className="text-[10px] font-black uppercase tracking-widest text-white/30">Attempt History</h3>
            </div>
            <div className="p-4 space-y-3">
                {attempts.map((a, i) => (
                    <div key={i} className="flex justify-between items-center text-sm font-bold">
                        <span className="text-white/40 tracking-tight">Attempt {a.attemptNumber || i + 1}</span>
                        <div className="flex items-center gap-3">
                            <span className={a.passed ? "text-emerald-400" : "text-rose-400"}>
                                {a.score !== null ? `${Math.round(a.score)}%` : a.status}
                            </span>
                            {a.passed ? <FaCheckCircle className="text-emerald-400 text-xs" /> : <FaTimes className="text-rose-400 text-xs" />}
                        </div>
                    </div>
                ))}
            </div>
