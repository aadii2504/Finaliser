...
export default function CourseStructureManager({ courseId, onClose, isEmbedded = false }) {
  ...
  const handleAddLesson = async (chapterId) => {
    ...
    if (!lessonData || !lessonData.title.trim()) return;
    ...
    try {
      ...
      await courseApi.lessons.create(courseId, chapterId, {
        ...
        order: (courseData.chapters.find(c => c.id === chapterId)?.lessons?.length || 0) + 1
      });
      setNewLesson(prev => ({ ...prev, [chapterId]: { title: "", description: "", duration: "" } }));
      await loadCourseStructure();
    } catch (err) {
  ...
  if (loading) {
    return (
      <div className={`${isEmbedded ? "py-10" : "fixed inset-0 bg-black/70 backdrop-blur-sm z-50"} flex items-center justify-center`}>
  ...
  if (courseId === "new") {
    return (
      <div className="p-8 text-center bg-slate-800/50 rounded-xl border border-slate-700">
        ...
        <p className="text-slate-400">Please save the course details first before managing its structure.</p>
      </div>
    );
  }
          ...
          {courseData?.chapters && courseData.chapters.length > 0 ? (
            <div className="space-y-4">
                ...
                >
                  ...
                  <div className="p-4 space-y-3 border-t border-slate-700">
                    ...
                    <div className="bg-slate-800/50 border border-slate-600 border-dashed rounded-lg p-4 mt-4">
                      <h4 className="text-sm font-semibold text-slate-200 mb-3">Add New Lesson</h4>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        <input
                          type="text"
                          placeholder="Lesson Title"
                          value={newLesson[chapter.id]?.title || ""}
                          onChange={(e) => updateNewLessonState(chapter.id, "title", e.target.value)}
                        ...
                        <input
                          ...
                          onChange={(e) => updateNewLessonState(chapter.id, "duration", e.target.value)}
                          className="px-3 py-2 bg-slate-900 border border-slate-600 rounded text-sm text-white focus:outline-none focus:ring-1 focus:ring-blue-500"
                        />
                        <textarea
                          placeholder="Lesson Description"
                          value={newLesson[chapter.id]?.description || ""}
                          onChange={(e) => updateNewLessonState(chapter.id, "description", e.target.value)}
                          rows="1"
                          className="md:col-span-2 px-3 py-2 bg-slate-900 border border-slate-600 rounded text-sm text-white focus:outline-none focus:ring-1 focus:ring-blue-500 resize-none"
                        />
                        <button
                          onClick={() => handleAddLesson(chapter.id)}
                          disabled={isCreatingLesson[chapter.id] || !newLesson[chapter.id]?.title?.trim()}
                        ...
                        >
                          {isCreatingLesson[chapter.id] ? "Adding..." : "+ Add Lesson"}
                        </button>
                      </div>
                    </div>
                  </div>
              ...
              <div className="bg-slate-800 border border-blue-500/30 border-dashed rounded-lg p-6 hover:border-blue-500/50 transition mt-6">
                <h3 className="text-lg font-semibold text-white mb-4">Add Another Chapter</h3>
            ...
            <div className="bg-slate-800 border border-blue-500/30 border-dashed rounded-lg p-6 hover:border-blue-500/50 transition text-center py-12">
              ...
              <p className="text-slate-400 mb-6">Create your first chapter to start building the course.</p>
              <form onSubmit={handleAddChapter} className="max-w-md mx-auto space-y-4">
                <input
                  type="text"
                  placeholder="Chapter Title"
                  value={newChapter.title}
