  ...
  return (
    <div className="mb-6 rounded-lg border border-[var(--border)] bg-[var(--card)] p-4">
      ...
      <div className="space-y-3 max-w-2xl grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="md:col-span-2">
          <label className="block text-sm sm:text-base mb-2">Thumbnail Image</label>
          <input
            ref={fileInputRef}
            type="file"
            accept="image/*"
          ...
          {form.thumbnail ? (
            ...
            <div
              ...
              className={`w-full rounded-lg border-2 border-dashed p-4 sm:p-6 md:p-8 transition-all duration-200 cursor-pointer flex flex-col items-center justify-center min-h-32 sm:min-h-40 md:min-h-48 ${dragActive
        ...
        <div className="md:col-span-2">
          <label className="text-sm font-medium opacity-70 block mb-2">Detailed Description (Visual Editor)</label>
          <VisualEditor
            value={form.description}
            onChange={(val) => setForm(f => ({ ...f, description: val }))}
            placeholder="Describe your course in detail..."
          />
          <p className="mt-2 text-[10px] opacity-40 italic">Formatting is applied directly in the editor above.</p>
          {errors.description && (
        ...
        <div className="md:col-span-2 flex gap-2 pt-2">
          <button
            onClick={onSave}
            disabled={!canSave}
            className={`px-3 py-2 rounded-md text-white ${canSave ? "bg-indigo-600" : "bg-indigo-600/40 cursor-not-allowed"
              }`}
          >
            Save
          </button>
